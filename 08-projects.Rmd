...
##### 2. The file `/class/datamine/data/fars/states.csv` contains the names of states along with their code. Read this file into a dataframe called `state_names` and use it to map `STATE` codes found in the `dat` dataset to the names of states by following the steps provided in the section **hint** below.

**Hints:**

- Make sure to first remove states from `state_names` that are not in the column `STATE` of the `dat` dataset, save the resulting data as `substate`.
- Create a new variable containing the `STATE` column (from the `dat` dataset) converted to a factor.
- Reorder `substate` by `code` using the `order` function.
- Use the `levels` function to set the levels of our new variable as the reordered state names in `substate`.

**Note:** In the next project, we will learn a much more effective way to accomplish this!

**Relevant topics:** [as.factor](#r-factors), [order](#r-order), [levels](#r-levels), [in](#how-do-i-get-the-values-in-list1-that-are-in-list2)
...
##### 3. In the previous project, we calculated how many accidents occured in 4 selected states, each month. If we wanted to extend this to every state, there would be more steps involved. The `tapply` function is a perfect fit for such a question. Use `tapply` to calculate the number of accidents (each row/observation is an accident) by month (`MONTH`) for each state (`STATE`). Which state has the most accidents?

**Relevant topics:** [tapply](#r-tapply), [which.max](#r-which)
...
##### 4. Use `tapply` to calculate the percentage of accidents during snowy weather and rainy weather. The image below provides the code given to each type of weather condition. Use the image to help you answer the question:

![](./images/weather.png)

**Hint:** You can solve this using `tapply` twice, or, you can wrap the two conditions you'd like to group by in a `list` by using the `list` function. We will learn more about lists later, however, a `list` is essentially an R obejct containing various types rather than a single type or data.
